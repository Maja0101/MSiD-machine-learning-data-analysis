...
for train_index, test_index in skf.split(X_poly, y):
    ...
    # model = MixtureOfExpertsSoft(
    #     base_model=CustomLogisticRegressionL1,
    #     expert_params={'lr': lr, 'epochs': iters, 'lambda_L1': reg},
    #     gater=LogisticRegression(max_iter=1000, multi_class='multinomial'))
    
    # model = MixtureOfExpertsHard(
    #     base_model=CustomLogisticRegressionL1,
    #     expert_params={'lr': lr, 'epochs': iters, 'lambda_L1': reg})
    
    model = MixtureOfExpertsSoftV2(
        base_model=CustomLogisticRegressionL1,
        gater=LogisticRegression(max_iter=1000, multi_class='multinomial'))
